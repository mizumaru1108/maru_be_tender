...
export default function AppointmentsTableRow({
  ...
  const handleReject = async (data: any) => {
    ...
    // console.log('reject data:', payload);
    onReject(payload);
  };

  const handleAccept = async () => {
    // console.log('accept id:', row.meetingId);
    onAccept(row.meetingId);
  };
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell align="left">
          {!isRequest ? (
            <Stack direction="row" flex={2} gap={2} justifyContent="center">
              {row.id.toLowerCase() === 'confirmed' ? (
                <Button
                  startIcon={
                    <div>
                      <svg
                  ...
                  variant="outlined"
                  onClick={() => {
                    window.open(row.appointmentLink, '_blank');
                  }}
                >
                  {/* بدء الاجتماع */}
                  {translate('appointment_table.button.start_meeting')}
                </Button>
              ) : (
                '-'
              )}
            ...
            <Stack direction="row" flex={2} gap={2} justifyContent="center">
              ...
              >
                {/* حذف */}
                {translate('appointment_table.button.reject')}
              </Button>
              <Button
                startIcon={<CheckIcon />}
                sx={{
              ...
              >
                {/* قبول الموعد */}
                {translate('appointment_table.button.accept')}
              </Button>
            </Stack>
          )}
        </TableCell>
