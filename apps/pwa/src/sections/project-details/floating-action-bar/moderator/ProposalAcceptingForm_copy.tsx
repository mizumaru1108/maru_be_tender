...
function ProposalAcceptingForm_copy({ onSubmit, onClose, loading }: FormProps) {
  ...
  const fetchingTracks = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const rest = await axiosInstance.get(`/tender/track/fetch-all`, {
...
export default ProposalAcceptingForm_copy;
