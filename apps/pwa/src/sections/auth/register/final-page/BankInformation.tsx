...
function BankInformation({
  ...
  data: BankingValuesProps;
  setStep: (val: number) => void;
}) {
  const [loading, setLoading] = useState<boolean>(false);
  const [bankValue, setBankValue] = useState<AuthorityInterface | null>(null);

  const getBankDetails = async () => {
    setLoading(true);

    try {
      const { status, data: resData } = await axios.get(
        `${TMRA_RAISE_URL}/tender/proposal/payment/find-bank-details`,
        {
          params: {
            id: data.bank_name,
          },
      ...
      <Stack direction="column" gap={2}>
        ...
        <Stack direction="row" justifyContent="space-between">
          <Stack direction="column" flex={1}>
            <Typography sx={{ color: '#93A3B0', fontSize: '15px' }}>رقم الحساب البنكي:</Typography>
            <Typography sx={{ fontSize: '18px' }}>{data.bank_account_number}</Typography>
          </Stack>
          <Stack direction="column" flex={1}>
            <Typography sx={{ color: '#93A3B0', fontSize: '15px' }}>اسم الحساب البنكي:</Typography>
            <Typography sx={{ fontSize: '18px' }}>{data.bank_account_name}</Typography>
          </Stack>
        </Stack>
        <Stack direction="column">
          <Typography sx={{ color: '#93A3B0', fontSize: '15px' }}>اسم البنك:</Typography>
          <Typography sx={{ fontSize: '18px' }}>
            {!loading && bankValue ? bankValue.bank_name : data.bank_name}
          </Typography>
        </Stack>
        <Stack direction="column">
