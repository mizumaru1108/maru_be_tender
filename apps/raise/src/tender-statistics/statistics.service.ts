...
export class TenderStatisticsService {
  ...
  async getAllTrackStatistics(from: any, to: any){
    ...
    governorates.map(governorate=>{
      if(!governorate.governorate || !governorate.outter_status || !governorate.project_track) return;
      if(!governorateObj[governorate.governorate]){
          nestedGovernorateObj[governorate.outter_status] = 1
          trackGovernorateObj[governorate.project_track] = Object.assign({}, nestedGovernorateObj);
          governorateObj[governorate.governorate] = Object.assign({}, trackGovernorateObj);
          delete nestedGovernorateObj[governorate.outter_status]
          delete trackGovernorateObj[governorate.project_track]
      }else{
        if(!governorateObj[governorate.governorate][governorate.project_track]){
          nestedGovernorateObj[governorate.outter_status] = 1
          trackGovernorateObj[governorate.project_track] = Object.assign({}, nestedGovernorateObj);
          governorateObj[governorate.governorate] = Object.assign(governorateObj[governorate.governorate], trackGovernorateObj);
          delete nestedGovernorateObj[governorate.outter_status]
          delete trackGovernorateObj[governorate.project_track]
      } else  {
          if (!governorateObj[governorate.governorate][governorate.project_track][governorate.outter_status]){
            nestedGovernorateObj[governorate.outter_status] = 1
            governorateObj[governorate.governorate][governorate.project_track] = Object.assign(governorateObj[governorate.governorate][governorate.project_track], nestedGovernorateObj);
            delete nestedGovernorateObj[governorate.outter_status]
          }else{
            governorateObj[governorate.governorate][governorate.project_track][governorate.outter_status] = governorateObj[governorate.governorate][governorate.project_track][governorate.outter_status] +1
          }
        }
      }
    })
    ...
    regions.map(region=>{
      if(!region.region || !region.outter_status || !region.project_track) return;
      if(!regionObj[region.region]){
        nestedRegionObj[region.outter_status] = 1
        trackRegionObj[region.project_track] = Object.assign({}, nestedRegionObj);
        ...
        if(!regionObj[region.region][region.project_track]){
          nestedRegionObj[region.outter_status] = 1
          trackRegionObj[region.project_track] = Object.assign({}, nestedRegionObj);
          regionObj[region.region] = Object.assign(regionObj[region.region], trackRegionObj);
          delete nestedRegionObj[region.outter_status]
          delete trackRegionObj[region.project_track]
        } else  {
          if (!regionObj[region.region][region.project_track][region.outter_status]){
            nestedRegionObj[region.outter_status] = 1
            regionObj[region.region][region.project_track] = Object.assign(regionObj[region.region][region.project_track], nestedRegionObj);
            delete nestedRegionObj[region.outter_status]
          }else{
            regionObj[region.region][region.project_track][ region.outter_status] = regionObj[region.region][region.project_track][region.outter_status] +1
          }
        }
      }
    })
    ...
    for (const [authority, userIds] of Object.entries(obj)) {
        ...
        authorities.map(authority=>{
            ...
            if(!authorityObj[authority.project_track]){
                ...
                authorityObj[authority.project_track] = Object.assign({}, nestedAuthorityObj);
                delete nestedAuthorityObj[authority.outter_status]
            }else{
                if(!authorityObj[authority.project_track][authority.outter_status]){
                    nestedAuthorityObj[authority.outter_status] = 1
                    authorityObj[authority.project_track] = Object.assign(authorityObj[authority.project_track], nestedAuthorityObj);
                    delete nestedAuthorityObj[authority.outter_status]
                } else  {
                    authorityObj[authority.project_track][ authority.outter_status] = authorityObj[authority.project_track][authority.outter_status] + 1
                }
            }
        })
    }
    ...
    tracks.map(track =>{
        ...
        if(!tracksObj[track.project_track]){
            ...
            if(!tracksObj[track.project_track][track.outter_status]){
                nestedTrackObj[track.outter_status] = 1;
                tracksObj[track.project_track] = Object.assign(tracksObj[track.project_track], nestedTrackObj);
                delete nestedTrackObj[track.outter_status]
            }else {
                tracksObj[track.project_track][track.outter_status] = tracksObj[track.project_track][track.outter_status] + 1;
            }
        }
