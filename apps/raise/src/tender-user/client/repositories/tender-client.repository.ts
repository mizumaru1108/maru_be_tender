  ...
  async findClientProposals(filter: SearchClientProposalFilter) {
    ...
    try {
      const response: any = await this.prismaService.$queryRaw`
        SELECT "user".id, "user".employee_name, "user".mobile_number, "user".email, "user".status_id, client_data.governorate, COUNT(proposal.id) AS proposal_count, COUNT(*) OVER() AS total_count
        FROM client_data
