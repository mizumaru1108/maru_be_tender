...
import { ProposalItemBudgetEntity } from '../entities/proposal.item.budget.entity';
...
export class UpdateItemBugetProps {
  id: string;
  proposal_id?: string;
  amount?: number;
  clause?: string;
...
export class ProposalItemBudgetRepository {
  private readonly logger = ROOT_LOGGER.child({
    'log.logger': ProposalItemBudgetRepository.name,
  });
  ...
  async update(
    ...
    try {
      const rawUpdatedItemBudget = await prisma.proposal_item_budget.update({
        where: { id: props.id },
        data: {
          amount: props.amount,
          clause: props.clause,
      ...
      const updatedItemBudgetEntity = Builder<ProposalItemBudgetEntity>(
        ProposalItemBudgetEntity,
        {
          ...rawUpdatedItemBudget,
          amount: parseFloat(rawUpdatedItemBudget.amount.toString()),
        },
      ).build();

      return updatedItemBudgetEntity;
    } catch (error) {
      this.logger.error(`error on updating item budget ${error}`);
      throw error;
    }
  }
  ...
  async deleteById(id: string, session?: PrismaService) {
    ...
    try {
      const rawUpdatedItemBudget = await prisma.proposal_item_budget.delete({
        where: { id },
      });
